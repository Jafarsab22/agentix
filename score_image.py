...
score_image.py
---------------
Includes location and no-location related scores; Pure detection + scoring helpers.
...
def score_single_card(card_cues: Iterable[str],
    ...
    NOTE: this function never injects Row/Column cues itself. If you want
    positional effects, pass them explicitly via extra_cues.
    """
...
def score_card_option_a(card_cues: Iterable[str],
    ...
    """
    Option A: linear utility U = Σ β_j x_j.

    card_cues + extra_cues are treated as 0/1 indicators; ln(price) enters
    as β_price * ln(price) if price is provided.
    """
    U = 0.0
    # binary cues
    for cue in set(card_cues) | set(extra_cues):
        p = params.get(cue)
        if p and p.get("beta") is not None:
            U += float(p["beta"])
    # continuous price term
    ln_price = _ln_or_none(price)
    if ln_price is not None:
...
def score_card_option_b(card_cues: Iterable[str],
                        ...
                        *,
                        ...
                        extra_cues: Iterable[str] = ()) -> float:
    """
    Option B: evidence-weighted readiness.

    Uses s_i = sign(β_i) * M_i * C_i * R_i (or precomputed s),
    readiness = Σ s_i / Σ (C_i * R_i), optionally price-weighted.
    """
    sum_s = 0.0
    sum_w = 0.0
