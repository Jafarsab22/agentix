...
Adds:
  ...
  • Per-trial audit of layout + mapped choice (results/ab_trials_<job_id>.jsonl).
...
def _ab_log_open(job_id: str) -> Path:
    """
    Ensure the per-trial audit file exists; return its path.
    Each line will be a JSON record with: t (trial#), layout[8], chosen ("A"/"B"/None).
    """
    p = RESULTS_DIR / f"ab_trials_{job_id}.jsonl"
    if not p.exists():
...
# ---------------- Image helpers ----------------
def _img_to_data_url(img: Image.Image, fmt: str = "JPEG", quality: int = 80) -> str:
    buf = io.BytesIO()
...
def _compose_grid_2x4(file_a: str, file_b: str, labels: list[str]) -> Tuple[str, list[list[str]]]:
    ...
    """
    assert labels.count("A") == 4 and labels.count("B") == 4, "layout must be 4×A/4×B"
...
def _openai_choose(image_b64: str, category: str = "", model_name: str | None = None) -> tuple[dict, dict]:
    ...
    # Preferred path: proper tool call
    if tcs:
    ...
    # Fallback: model placed JSON directly in content
...
def _rc_from_args(args: dict) -> tuple[int | None, int | None]:
    """
    Robustly extract zero-based (row, col) from varied shapes: direct row/col, single index,
    nested blocks, or string patterns. Returns (None, None) if not recognised.
    """
    ...
    # nested tool-args as JSON string/dict
    ...
    # string patterns (r2c3 / 'row 2, col 3' / 'top-left')
    for v in list(args.values()):
        if isinstance(v, str):
            ...
            if any(w in s for w in ("top", "bottom", "left", "right")):
                ...
                cc = 0 if "left" in s else (2 if "center" in s else (3 if "far right" in s else (1 if "right" in s else None)))
                if rr is not None and cc is not None:
                    return rr, cc
...
def _trial_once(
    ...
    model_name: str | None,
    dbg_file,
) -> tuple[str | None, list[list[str]], dict]:
    labels = ["A"] * 4 + ["B"] * 4
    ...
    chosen: str | None = None if (r0 is None or c0 is None) else layout[r0][c0]

    # per-trial diagnostics (tool/parse info)
    try:
        dbg_file.write(
            json.dumps(
                {
...
# ---------------- Background worker ----------------
def _bg_run_live_ab(
    job_id: str, file_a: str, file_b: str, n_trials: int, category: str | None, model_name: str | None
):
    try:
        a = b = invalid = 0
        diag_counts = {"api_calls": 0, "tool_missing": 0, "parse_error": 0, "null_coords": 0, "oob_coords": 0}
        examples: list[dict[str, Any]] = []  # keep up to 5 examples

        audit_path = _ab_log_open(job_id)  # layout/choice audit file
        ...
        with open(dbg_path, "a", encoding="utf-8") as dbg:
            for t in range(1, n_trials + 1):
                ...
                # write compact audit line: layout + mapped choice
                try:
                    layout_flat = [lab for row in layout for lab in row]  # 8 items row-major
                    rec = {"t": t, "layout": layout_flat, "chosen": chosen}
                    with open(audit_path, "a", encoding="utf-8") as fh:
                        fh.write(json.dumps(rec, ensure_ascii=False) + "\n")
                except Exception:
                    pass

                # retain a few recent examples for the JSON summary
                if len(examples) < 5:
                    examples.append(
                        {
